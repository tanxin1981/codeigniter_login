...
class User extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->redisClient = $client = new Predis\Client([
            'scheme' => 'tcp',
            'host'   => 'localhost',
            'port'   => 6379,
            'database' => 1
	...
	public function register() {
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->user_model->create_user($username, $email, $password)) {
                $this->user_model->save_user_history($this->redisClient, $username, 'register');
				// user creation ok
				$this->load->view('header');
	...
	public function login() {
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->user_model->resolve_user_login($username, $password)) {
                ...
                $this->user_model->save_user_history($this->redisClient, $username, 'login');
	...
	public function history() {
        if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
            ...
            $this->load->view('user/history/history', $data);
